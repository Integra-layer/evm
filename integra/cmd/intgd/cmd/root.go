...
func initRootCmd(rootCmd *cobra.Command, evmApp *integra.EVMD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	if height != -1 {
		exampleApp = integra.NewExampleApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = integra.NewExampleApp(logger, db, traceStore, true, appOpts, baseapp.SetChainID(chainID))
	}
